...
```
Add a Data property to the DataPage.xaml.vb:
```vb
...
Add a Data property to the DataViewModel.vb:
```vb
Private _data As String
...
Subscribe to the OnBackgroundEntering event when navigating to the Page and unsubscribe when navigating from the Page in the Data.xaml.vb file:
```vb
