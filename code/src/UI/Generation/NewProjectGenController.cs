        ...
        private static void TrackTelemery(IEnumerable<GenInfo> genItems, Dictionary<string, TemplateCreationResult> genResults, double timeSpent, string appProjectType, string appFx, string language)
        {
            try
            {
                ...
                foreach (var genInfo in genItems)
                {
                    ...
                    {
                        AppHealth.Current.Telemetry.TrackProjectGenAsync(genInfo.Template, appProjectType, appFx, genResults[resultsKey], GenContext.ToolBox.Shell.GetVsProjectId(), language, pagesAdded, featuresAdded, pageIdentities, featureIdentities, timeSpent, GenContext.Current.PerformanceCounters).FireAndForget();
                    }
                    else
                    {
