...
namespace Microsoft.Templates.Core.PostActions
{
    public abstract class PostActionFactory
    {
        ...
        {
            return Path.GetExtension(f).Equals(".xaml", StringComparison.OrdinalIgnoreCase) .ToLowerInvariant() == ".xaml" & File.ReadAllText(f).StartsWith(MergeConfiguration.ResourceDictionaryMatch, StringComparison.Ordinal);
        }
    }
}
