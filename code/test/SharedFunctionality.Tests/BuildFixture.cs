...
namespace Microsoft.Templates.Test
{
    public sealed class BuildFixture : BaseGenAndBuildFixture, IDisposable
    {
        ...
        private static void InitializeTemplates(TemplatesSource source)
        {
            if (!syncExecuted)
            {
                GenContext.Bootstrap(source, new FakeGenShell(Platforms.Uwp, ProgrammingLanguages.CSharp), Platforms.Uwp, ProgrammingLanguages.CSharp, TestConstants.TemplateVersionNumber);
