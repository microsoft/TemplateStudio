...
namespace Microsoft.Templates.Test.Build.Wpf
{
    ...
    public class BuildPrismProjectTests : BaseGenAndBuildTests
    {
        ...
        public async Task Build_All_G2_ProjectNameValidation_Wpf(string projectType, string framework, string platform, string language)
        {
        ...
        [Trait("Type", "BuildAllPagesAndFeaturesWpf")]
        [Trait("Type", "BuildRandomNamesWpf")]
        public async Task Build_All_G1_ProjectNameValidation_Wpf(string projectType, string framework, string platform, string language)
        {
            Func<ITemplateInfo, bool> templateSelector =
                t => t.GetTemplateType().IsItemTemplate()
                && (t.GetProjectTypeList().Contains(projectType) || t.GetProjectTypeList().Contains(All))
