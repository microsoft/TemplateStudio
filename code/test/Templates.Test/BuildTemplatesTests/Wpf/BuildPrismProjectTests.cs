...
namespace Microsoft.Templates.Test.Wpf
{
    ...
    public class BuildPrismProjectTests : BaseGenAndBuildTests
    {
        ...
        [Theory]
        [MemberData(nameof(BaseGenAndBuildTests.GetPageAndFeatureTemplatesForBuild), "Prism", ProgrammingLanguages.CSharp, Platforms.Wpf)]
        [Trait("ExecutionSet", "BuildOneByOnePrismWpf")]
        [Trait("ExecutionSet", "_OneByOne")]
        [Trait("Type", "BuildOneByOnePrismWpf")]
        public async Task BuildPrismOneByOneItemsWpfAsync(string itemName, string projectType, string framework, string platform, string itemId, string language)
        {
