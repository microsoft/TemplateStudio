MSTest 是 Microsoft 的單元測試工具套件。此單元測試framework會定義在 Microsoft.VisualStudio.QualityTools.UnitTestFramework.dll 中。使用單元測試framework建立的單元測試可以在 Visual Studio 中執行，或從命令列使用 MSTest.exe 執行。
